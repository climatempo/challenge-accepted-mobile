...
void main() {
  ...
  group('Fetch cities', () {
    test('should return an List CityModel', () async {
      SharedPreferences.setMockInitialValues({'LISTCITIESKEY': _json});

      final result = citiesRepository.fetchCities();
      final listCity = await result;
      expect(listCity, isA<List<CityModel>>());
      expect(listCity.first.id, 8285);
      expect(listCity.first.name, 'Abadia de Goiás');
    });

    test('fetch local cities', () async {
      SharedPreferences.setMockInitialValues({'LISTCITIESKEY': _json});
      List result = await citiesRepository.fetchCitiesLocal();
      expect(result.first['id'], 8285);
      expect(result.first['name'], 'Abadia de Goiás');
    });

    test('save cities list', () async {
      SharedPreferences.setMockInitialValues({'LISTCITIESKEY': _json});
      Future<void> save = citiesRepository.saveCitiesList([{}]);
      expect(save, completes);
    });
  });

  group('City Forecast', () {
    test('Should return an CityForecastModel', () async {
      SharedPreferences.setMockInitialValues(
          {'CITYFORECASTKEY': _jsonCityForecast});

      final cityForecastModel =
          await citiesRepository.fetchCityForecast('3477');
      expect(cityForecastModel.name, 'São Paulo');
      expect(cityForecastModel.state, 'SP');
    });

    test('should return local city forecast saved', () async {
      SharedPreferences.setMockInitialValues(
          {'CITYFORECASTKEY': _jsonCityForecast});
      final cityForecast = citiesRepository.fetchCityForecastLocal();

      expect(cityForecast, completes);
    });

    test('save city forecast local', () async {
      SharedPreferences.setMockInitialValues(
          {'CITYFORECASTKEY': _jsonCityForecast});

      var save =
          citiesRepository.saveCityForecast(jsonDecode(_jsonCityForecast));
      expect(save, completes);
    });
  });

  group('Cities Historic', () {
    test('get cities historic', () async {
      SharedPreferences.setMockInitialValues({'CITIESHISTORICKEY': _json});

      final result = citiesRepository.getCitiesHistoric();
      final listCity = await result;
      expect(listCity, isA<List<CityModel>>());
      expect(listCity.first.id, 8285);
      expect(listCity.first.name, 'Abadia de Goiás');
    ...
    test('save city in historic', () async {
      SharedPreferences.setMockInitialValues({'CITIESHISTORICKEY': _json});
      var save = citiesRepository.saveCityInHistory({
        "id": 8285,
    ...
    test('clear historic', () async {
      SharedPreferences.setMockInitialValues({'CITIESHISTORICKEY': _json});
      var clear = citiesRepository.clearAllHistoric();
