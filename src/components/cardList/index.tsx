...
const CardList = () => {
  ...
  const [sourceOfList, setSourceOfList] = useState<'storage' | 'api' | null>(
    null,
  );
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const latestCity = JSON.parse(
          (await AsyncStorage.getItem('latest')) || '{}',
        );
        ...
        if (isEmpty(latestCity)) {
          ...
          setCityForecastData(
            (await getForecast(latestCity.id, Config.TOKEN)).data,
          );
        }
  ...
  const getCitiesFromStorage = async () => {
    setFilteredDataSource(
      (await searchHistoricModel.getAll()).map(city => JSON.parse(city.data)),
    );
  ...
  const renderSearchCityList = async (item: any) => {
    try {
      ...
      await cityForecastModel.saveCityForecast(
        item.id,
  ...
  return (
    <>
      <S.Container>
        ...
        <FlatList
          ListFooterComponent={<S.FooterContainer />}
          ListHeaderComponent={
            <S.InfoText>
              {cityForecastData.name} {cityForecastData.state}
            </S.InfoText>
          }
